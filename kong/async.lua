...
local function after_timer(premature, self)
  ...
  while head ~= tail do
    ...
    if bucket.head == bucket.tail then
      ...
      while bucket.head ~= bucket.tail do
        ...
        local ok, err = bucket.jobs[bucket_tail][2](self)
        if not ok then
          kong.log:err(err)
          break
        end
...
local function create_after_job(job)
  return function(self)
...
function async.new()
  ...
  return setmetatable({
    ...
    lawn = {
      head    = 0,
      tail    = 0,
      ttls    = kong.table.new(BUCKET_SIZE, 0),
      buckets = {},
    },
    buckets = {},
    closest = 0,
...
function async:init_worker()
  ...
  ok, err = ngx.timer.every(TIMER_INTERVAL, after_timer, self)
  if not ok then
    return nil, err
  end
...
function async:after(delay, func, ...)
  ...
  bucket.jobs[bucket.head] = {
    expiry,
    create_after_job(create_job(func, ...)),
  }
